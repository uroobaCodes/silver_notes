...
function DeleteNoteButton({ noteId, deleteNoteLocally }: Props) {
  ...
  const titleSuccess = "Note Deleted";
  const descriptionSuccess = "You have successfully deleted the note";
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button
          className="group-hover/item:opacty-100 absolute top-1/2 right-2 size-7 -translate-y-1/2 p-0 opacity-0 group-hover/item:opacity-100 [&_svg]:size-3"
          variant="ghost"
        >
          <Trash2 />
        </Button>
